...
export function createReviewProvider(config) {
    ...
    if (config.provider.kind === "azure-openai") {
        return new AzureOpenAIReviewProvider(config.provider.apiKey, config.provider.endpoint, config.provider.deployment);
    }
    ...
    return new OpenAICompatibleReviewProvider(config.provider.baseUrl, config.provider.model, config.provider.apiKey);
}
